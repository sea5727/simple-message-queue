...
{
    class ServerSession2 : public std::enable_shared_from_this<ServerSession2>{
    private:
    ...
    private:
        ...
        void
        do_read_body( unsigned int bodylen) {
            ...
            boost::asio::async_read( socket,  boost::asio::buffer(boost::asio::buffer(buffer) + FRAME_SIZE), boost::asio::transfer_exactly(bodylen - FRAME_SIZE),
                [self, this](const boost::system::error_code & error, std::size_t len){
